...
// Tipagem declarada internamente para evitar erro de importação
...
export default function App() {
  ...
  const loadAppContext = useCallback(async () => {
    try {
      ...
      console.error('Erro crítico no App:', err.message);
      setError('Sincronização falhou. Verifique sua conexão.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#FAF8F4] flex flex-col items-center justify-center p-10 text-center">
        ...
        <h2 className="text-[#2D3142] font-black text-xl mb-2 italic">Conexão Interrompida</h2>
        <p className="text-slate-500 text-sm mb-8 font-medium">{error}</p>
        <button onClick={loadAppContext} className="px-10 py-5 bg-[#2D3142] text-white rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest shadow-2xl active:scale-95 transition-all">Tentar Novamente</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0F172A] flex justify-center items-center p-0 sm:p-6">
      <main className="w-full max-w-[430px] h-full sm:h-[932px] bg-[#FAF8F4] shadow-2xl overflow-hidden relative sm:rounded-[4rem] border-[8px] border-[#1E293B] flex flex-col">
        <div className="flex-1 overflow-y-auto no-scrollbar relative">
          {screen === 'welcome' && <Welcome onNext={() => navigateTo('elderly')} />}
          {screen === 'elderly' && <ElderlyRegistration familyId={userProfile?.family_id} onNext={loadAppContext} />}
          {screen === 'family' && <FamilyMembers familyId={userProfile?.family_id} onNext={() => navigateTo('dashboard')} onBack={() => navigateTo('elderly')} />}
          {screen === 'dashboard' && <Dashboard user={userProfile} elderly={elderly} family={familyData} onNavigate={navigateTo} />}
          {screen === 'timeline' && <DailyTimeline elderly={elderly} user={userProfile} onNavigate={navigateTo} />}
          {screen === 'health' && <Health elderly={elderly} onNavigate={navigateTo} />}
          {screen === 'finance' && <Finance elderly={elderly} familyId={userProfile?.family_id} user={userProfile} onNavigate={navigateTo} />}
        </div>
...
function NavItem({ active, onClick, icon, label }: any) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${active ? 'text-[#4A7FA5] scale-110' : 'text-slate-300 hover:text-slate-400'}`}>
      <div className={`p-1.5 rounded-xl ${active ? 'bg-[#4A7FA5]/10' : ''}`}>{icon}</div>
